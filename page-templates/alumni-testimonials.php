					...
					<div class="cell testimonial-container">
						<div class="card testimonial">
							...
							<div class="card-section testimonial-bio">
								...
								<h3><?php echo esc_html( get_post_meta( $post->ID, 'ecpt_job', true ) ); ?></h3>
								<?php endif; ?>
								<?php if ( get_post_meta( $post->ID, 'ecpt_class', true ) ) : ?>
								<h3><span class="light">Class of <?php echo esc_html( get_post_meta( $post->ID, 'ecpt_class', true ) ); ?></span></h3>
								<?php endif; ?>
							...
							<div class="reveal testimonial-content" id="post-<?php the_ID(); ?>" aria-labelledby="Modal-<?php the_ID(); ?>" data-reveal>
								...
								<?php if ( get_post_meta( $post->ID, 'ecpt_internship', true ) ) : ?>
									<p><strong>Internship:</strong> <?php echo esc_html( get_post_meta( $post->ID, 'ecpt_internship', true ) ); ?></p>
								<?php endif; ?>
								<?php if ( get_post_meta( $post->ID, 'ecpt_job', true ) ) : ?>
									<p><?php echo esc_html( get_post_meta( $post->ID, 'ecpt_job', true ) ); ?></p>
								<?php endif; ?>
								<?php if ( get_post_meta( $post->ID, 'ecpt_class', true ) ) : ?>
									<p>Class of <?php echo esc_html( get_post_meta( $post->ID, 'ecpt_class', true ) ); ?></p>
								<?php endif; ?>
								<?php the_content(); ?>
								</div>
