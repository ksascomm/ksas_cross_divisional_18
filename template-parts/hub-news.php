...
if ( is_array( $hub_articles ) ) {
	foreach ( $hub_articles['articles'] as $hub_article ) {
		?>
<article class="post-listing hub-article" aria-labelledby="post-<?php echo esc_html( $hub_article['id'] ); ?>">
	<header>
		<?php
		$date = $hub_article['publish_date'];
		?>
		<time class="updated" datetime="<?php echo esc_html( $hub_article['publish_date'] ); ?>"><?php echo gmdate( 'F d, Y', $date ); ?></time>
		<h2><a href="<?php echo esc_url( $hub_article['url'] ); ?>" id="post-<?php echo esc_html( $hub_article['id'] ); ?>"><?php echo esc_html( $hub_article['headline'] ); ?></a></h2>
	</header>
	<div class="entry-content">
		<div class="grid-x">
			<div class="small-6 large-4 cell">
				<img width="360" height="240" class="news-thumb" src="<?php echo esc_url( $hub_article['_embedded']['image_thumbnail'][0]['sizes']['thumbnail'] ); ?>" alt="From The Hub: <?php echo esc_html( $hub_article['headline'] ); ?>" />
			</div>
